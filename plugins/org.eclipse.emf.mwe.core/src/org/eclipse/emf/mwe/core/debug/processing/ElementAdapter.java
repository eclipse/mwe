...
/**
 * An adapter to the <code>DebugMonitor</code> that handle all element type
 * specific activities on the runtime side.<br>
 * That is process specific decisions and syntax element and variable
 * normalisation.
 * 
 */
public interface ElementAdapter {
	...
	/**
	 * this method is called from the DebugMonitor prior to all other suspension
	 * activities (e.g. breakpoint check, subroutine level check)<br>
	...
	/**
	 * this method is called from the DebugMonitor after all other suspension
	 * activities (e.g. breakpoint check, subroutine level check)<br>
	 * this provides the opportunity for a last call by the adapter.<br>
	 * Implementations can *avoid* suspension, although it would suspend
	 * otherwise, if the return value is set to false here
	 * 
	 * @param lastState
	 *            the consolidated decisions previously asked
	 * @param element
	 *            the element that is going to be debugged
	 * @param flag
	 *            one of the values <code>IProcessHandler.PUSH</code> or
	 *            <code>IProcessHandler.POP</code>
	 * @return whether or not to suspend the debugger process for the element
	 */
	 ...
	 * the element name should be the name of the surrounding element (e.g. in
	 * Java we highlight the line to be debugged, but show the surrounding
	 * method name in the Launch view)<br>
	 * start, end and line should point to the text that shall be highlighted
	 * for the element
	 * 
	 * @param element
	 *            the element that is going to be debugged
	 * @return a SyntaxElement that contains the information to be shown in the
	 *         debugger views in a common neutral way
	 */
	public SyntaxElement createElementTO(Object element);

	/**
	 * returns whether the element has a body and debugger shall stop again at
	 * the end of the execution of that element
	 * 
	 * @param element
	...
	/**
	 * computes a SyntaxElement for a surrounding element representing the end
	 * position to be highlighted before returning<br>
	...
	/**
	 * collect all available variables for a given element. With this method
	 * only the variables themselfs are collected, not their members.<br>
	 * Implementations must consider that <code>element</code>can be both a
	 * syntax element or an arbitrary java element to collect the member
	 * variables from!!
	 * 
	 * @param element
	 *            the current element
	...
	/**
	 * compute the string representation of the element ready to display in the
	...
	/**
	 * compute a shorter string representation of the element ready to display
	 ...
	 * the context should be cached and can be used to collect information about
	 * elements
	 * 
	 * @param context
	 *            the context
