...
public class WorkflowReconcilingStrategy implements IReconcilingStrategy,
            ...
            while (nextPos < rangeEnd) {
                ...
                switch (ch) {
                    case '<':
                        ...
                        String tagString = document
                                .get(startOffset, Math.min(nextPos
                                        - startOffset, rangeEnd - startOffset));
                        newLines += newLines;

                        final int res = addPosition(depth, newLines,
                                startOffset, startNewLines, classification);
                    ...
                    case '\'':
                        ...
                        tagString = document.get(startOffset, Math.min(nextPos
                                - startOffset, rangeEnd - startOffset));
                        newLines += newLines;
                        nextPos++;
                        while (nextPos < rangeEnd) {
    ...
    private int addPosition(final int depth, final int newLines,
            final int startOffset, final int startNewLines,
            final int classification) throws BadLocationException {
        int nl = newLines;
        switch (classification) {
            case START_TAG:
                nl += recursiveTokens(depth + 1);
                if (nl > startNewLines + 1) {
                    emitPosition(startOffset, nextPos - startOffset);
                }
                break;
            case LEAF_TAG:
