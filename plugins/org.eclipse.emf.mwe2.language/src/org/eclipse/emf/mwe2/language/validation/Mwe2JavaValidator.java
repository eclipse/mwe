...
public class Mwe2JavaValidator extends AbstractMwe2JavaValidator {
	...
	@Check
	public void checkUnusedReferable(Module referable) {
		TreeIterator<EObject> iterator = referable.eResource().getAllContents();
		Set<Referrable> referenced = Sets.newHashSet();
		Set<Referrable> declared = Sets.newHashSet();
		while (iterator.hasNext()) {
			EObject next = iterator.next();
			if (next instanceof Referrable) {
				if (((Referrable) next).getName() != null)
					declared.add((Referrable) next);
			} else if (next instanceof Reference) {
				referenced.add(((Reference) next).getReferable());
			}
		}
		declared.removeAll(referenced);
		for (Referrable referrable : declared) {
			warning("Variable '" + referrable.getName() + "' is unused.",
